...
public class Main {
	...
	private static void processListCommand() {
		MemberListService memberListService = assembler.getSelectListService();
		List<Member> list = memberListService.selectList();
		if (list.size() == 0) {
	...
	private static void processInfoCommand(String[] args) {
		if (args.length != 2) {
		 ...
		 MemberInfoService memberInfoService = assembler.getMemberInfoService();
		 try {
